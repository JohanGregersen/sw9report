...
Test case 1, is visualized in \ref{fig:shorttrips} and represents two $\sim$8.1km equal trips, driven from an original destination to a target-destination and back with the same car. The route driven is simple and has very few turns. One trip is colored green, the other is colored red. The spatial information is not available to a trip-entry, therefore the digital roadmap layer is omitted.
...
Test case 2 can be seen in figure \ref{fig:longtrips}. These are longer and more complex trips between $\sim$31.7km and $\sim$32.0km in distance. The trips involve two different cars. 
